...
export function useImageLabeler() {
  ...
  const [dirHandle, setDirHandle] = createSignal<string | null>(null);
  ...
  async function loadImageByIndex(index: number) {
    ...
    const dirPath = dirHandle();

    if (index < 0 || index >= images.length || !dirPath) {
    ...
    const file = await getImageFileFromDirectory(dirPath, fileName);
    const dataUrl = await readImageAsDataURL(file);
    setImageUrl(dataUrl);

    const annotation = await loadAnnotationFile(dirPath, fileName);

    let loadedCategories: Category[] = [];
    if (annotation && annotation.categories && annotation.categories.length > 0) {
      loadedCategories = annotation.categories;
      setCategories(loadedCategories);
      nextCategoryId = Math.max(...loadedCategories.map((c) => c.id)) + 1;
  ...
  async function saveLabels() {
    ...
    const currentDirPath = dirHandle();

    if (currentDirPath) {
      const blob = new Blob([JSON.stringify(annotation, null, 2)], {
        type: "application/json",
      });
      await writeFileToDirectory(currentDirPath, suggestedName, blob);
    } else {
  ...
  async function exportCrops(config: CropConfig, dirPath: string): Promise<CropResult> {
    if (!imagePath()) {
    ...
    const img = new Image();
    img.src = imageUrl();
    const { promise: loadPromise, resolve: loadResolve } = Promise.withResolvers<void>();
    ...
    for (let y = 0, row = 0; y < imgHeight; y += strideY, row++) {
      ...
      for (let x = 0, col = 0; x < imgWidth; x += strideX, col++) {
        ...
        const { promise: blobPromise, resolve: blobResolve } = Promise.withResolvers<Blob>();
        canvas.toBlob((b) => {
          blobResolve(b ?? new Blob([]));
        }, "image/png");
        const blob = await blobPromise;
        await writeFileToDirectory(dirPath, `${tileName}.png`, blob);
        ...
        await writeFileToDirectory(dirPath, `${tileName}.json`, jsonBlob);
